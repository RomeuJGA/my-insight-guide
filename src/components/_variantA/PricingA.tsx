            ...
            {packages.map((plan) => {
              ...
              return (
                ...
                >
                  {plan.badge && (
                    <div
                      className={`absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 rounded-full text-xs font-medium ${
                        highlight
                          ? "bg-[hsl(40_30%_98%)] text-[hsl(165_35%_28%)]"
                          : "bg-[hsl(165_35%_28%)] text-[hsl(40_30%_98%)]"
                      }`}
                    >
                      {plan.badge}
                    </div>
                  ...
                  {plan.description && (
                    <p className={`text-sm italic mb-4 ${highlight ? "opacity-75" : "text-muted-foreground"}`}>
                      {plan.description}
                    </p>
                  )}
                  ...
                  <p className={`text-sm mb-8 ${highlight ? "opacity-70" : "text-muted-foreground"}`}>
                  ...
                  <ul className="space-y-3 mb-8">
                    {[
                      ...
                      <li key={f} className="flex items-start gap-3 text-sm">
                        <Check
                          className={`w-4 h-4 mt-0.5 shrink-0 ${highlight ? "" : "text-[hsl(165_35%_28%)]"}`}
                        />
                        <span className={highlight ? "opacity-90" : "text-foreground/80"}>{f}</span>
                      </li>
                    ))}
                  </ul>
